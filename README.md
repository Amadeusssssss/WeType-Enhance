...
本项目严格遵循开源衍生分支版本规范，与上游主干版本保持清晰血统关系：

- **基线血统**：基于上游 `v1.28.4` 衍生，首发版本为 `v1.28.4-18key`（内部 `versionCode = 12804`）。
- **小版本迭代**：若在当前基线继续发布修复或功能改进，按 `v1.28.4-18key.1`、`v1.28.4-18key.2` 顺延；
- **跟进上游**：若未来跟进合入上游新大版本（例如 `v1.28.6`），版本号将自然进阶为 `v1.28.6-18key`。

---

## 自动化构建与 CI/CD

仓库配置了全自动 GitHub Actions 工作流：

1. **模块 APK 构建 (`Build Module`)**：
   - 支持在 Actions 页面手动一键触发（`workflow_dispatch`）；
   - 自动编译 Release APK、签名对齐，并自动创建或发布到对应的 GitHub Release；
   - 构建成功后可自动触发修补包流水线。

2. **免 Root 修补包构建 (`Build Patched APK`)**：
   - **联动触发**：模块 APK 构建完成后自动运行，无头动态注入 LSPatch 并签名，产出免 Root 修补版输入法 APK 并自动追加挂载至该 Release；
   - **官方更新自动监听**：每日定时轮询微信输入法官方发布源（`https://z.weixin.qq.com/`），若检测到 Android 新版本发布，自动拉取底包打补丁并发布；
   - **手动构建**：支持在 GitHub Actions 页面直接填入模块 Tag 或底包直链一键制作修补包。

---

## 从源码构建

准备 JDK 21 与 Android SDK，然后执行：
